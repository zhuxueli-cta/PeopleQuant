...
print(f"验证下单价格错误,错误信息:{r['last_msg']},下单价格:{quote['LowerLimitPrice']+0.5}非最小跳动价位:{symbole_info['PriceTick']}的整数倍")
r = pqapi.open_close(symbol,"pingduo",1000,quote["BidPrice1"],order_info='验证平仓数量不足',ctp_error=True) #平仓手数不足,验仓
print(f"验证平仓数量不足,错误信息:{r['last_msg']}")
r = pqapi.open_close(symbol,"kaiduo",0,quote["BidPrice1"],open_min_volume=8,order_info='验证单笔最小下单数量') #低于单笔最小下单数量
...
print(r['last_msg'])
r = pqapi.open_close(symbol,"pingkong",1,quote["UpperLimitPrice"],order_info='再次平仓',ctp_error=True) #
print(r['last_msg'])
