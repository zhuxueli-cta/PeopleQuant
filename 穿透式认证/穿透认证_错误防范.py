...
r = pqapi.open_close(symbol,"kaiduo",500,quote["AskPrice1"],order_info='验证资金不足',ctp_error=True) #验证资金不足
print(f"验证资金不足,错误信息:{r['last_msg']}")
r = pqapi.open_close(symbol,"kaiduo",500,quote["AskPrice1"],order_info='验证资金不足',ctp_error=True) #验证资金不足
print(f"验证资金不足,错误信息:{r['last_msg']}")
r = pqapi.open_close(symbol,"kaiduo",50,quote["AskPrice1"],order_info='验证资金不足',ctp_error=True) #验证资金不足
print(f"验证资金不足,错误信息:{r['last_msg']}")
r = pqapi.open_close(symbol,"kaiduo",500,quote["AskPrice1"],order_info='验证资金不足',ctp_error=True) #验证资金不足
print(f"验证资金不足,错误信息:{r['last_msg']}")
r = pqapi.open_close(symbol,"kaiduo",500,quote["AskPrice1"],order_info='验证资金不足',ctp_error=True) #验证资金不足
