...
for i in range(5):
    #开仓,多头1手
    r = pqapi.open_close(symbol,'kaiduo',1,quote["UpperLimitPrice"],ctp_error=True,order_info='开仓')
    #开仓,多头1手,验证撤单
    r = pqapi.open_close(symbol,'kaiduo',1,quote["LowerLimitPrice"],n_price_tick=0,che_time=2,order_info='撤单')
    #获取报撤单、信息量统计
    orderrisk = pqapi.get_order_risk(symbol)
    print(f"下单完成,合约:{r['symbol']},报单次数:{orderrisk['order_count']},撤单次数:{orderrisk['cancel_count']},重复报单次数:{orderrisk['repeat']}")
